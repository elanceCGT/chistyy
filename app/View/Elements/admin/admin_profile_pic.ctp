...
<button type="button" class="btn btn-primary btn-lg" id="openCropDiv" data-toggle="modal" data-target="#myModal" style="display:none">
  Launch demo modal
</button>
<!-- Modal -->
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
            ...
            <!--<div class="clearDiv"><?php echo $this->Html->image('loadingsubimg_2.gif',array('id'=>'image_sub')); ?></div>-->
...
$(document).ready(function(){
    ...
    new AjaxUpload(btnUpload, {
        action: '<?php echo $this->Html->url( array("admin"=>true,"controller" => "images", "action" => "uploadphoto"));?>',
        //data : {    user_id : '<?php // echo $this->request->data['User']['id'];?>' },
        name: 'uploadfile',         
        ...
        onComplete: function(file, response){
            ...
            $("#cropbox1,#preview").attr("src",PathUrl+'images/temp/'+imageName);
            setTimeout("initializingCrop()",1000);  
            $("#openCropDiv").trigger("click");
...
function setMyCode(mySrc){
        ...
        $.ajax({
            type: "POST",
            async : false,
            url : '<?php echo $this->Html->url(array("admin"=>true,"controller" => "images","action" => "save_profileimg"));?>',
            data : { mycode : mycode,x:x,y:y,x2:x2,y2:y2,w:w,h:h},
            success : function(retData){
                    var s = "<?php echo $this->webroot.'images/user/small/';?>"+retData;
                   $("#profileImgSmall").attr("src",s);
