<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-lg-2">
    	<a style="margin-top:20px;" href="<?php echo $this->Html->url(array('controller' => 'serviceprovider', 'action' => 'admin_addservice',$serviceproviderid)); ?>" class="btn btn-primary control-label pull-right">Add</a>
    </div>
</div>
...
<div id="dialog-confirm" title="Delete Service Provider?" style="display:none;">
	<p>
		<span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Are you sure You want to delete this Service Provider?
	</p>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#list").jqGrid({
			url: '<?php echo $this->Html->url(array("controller" => "serviceprovider", "action" => "admin_SPServicelist", $serviceproviderid)); ?>',
			datatype: "json",
			mtype: 'GET',
			...
			sortname: 'ServiceProviderService.id',
			sortorder: 'asc',
			viewrecords: true,
			gridview: true,
			hidegrid: false,
			...
			resizable: false,
	     	...
	     	buttons: {
	     		"Delete Service": function() {
	     			$(this).dialog("close");
					URL = '<?php echo $this->Html->url(array("controller" => "serviceprovider","action" => "deleteservice"));?>';
					$.ajax({
						url : URL,
						type: "POST",
						data : ({id:id}),
