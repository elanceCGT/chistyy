...
<div id="dialog-confirm" title="Delete Category?" style="display:none;">
  <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Are you sure You want to Delete this Category?</p>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() { 
		$("#list").jqGrid({
			url: '<?php echo $this->Html->url(array("controller" => "booking", "action" => "admin_mainbookinglist")); ?>',
			datatype: "json",
			mtype: 'GET',
			//colNames: ['S No','Category', 'No of Service', 'Add New Service', 'Status', 'Edit', 'Delete'],
			colNames: ['S No','User', 'Status', 'Edit', 'Delete'],
			colModel: [
	...
	function changeCategoryStatus(id,status){
		URL = '<?php echo $this->Html->url(array("controller" => "category","action" => "admin_changestatusmain"));?>';
		$.ajax({
			url : URL,
			type: "POST",
		...
		$( "#dialog-confirm" ).dialog({
			...
			buttons: {
				"Delete Category": function() {
					$( this ).dialog( "close" );
					URL = '<?php echo $this->Html->url(array("controller" => "category","action" => "admin_deletemain"));?>';
					$.ajax({
						url : URL,
						type: "POST",
						data : ({id:id,status:status}),
