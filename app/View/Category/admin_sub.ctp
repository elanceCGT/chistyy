<div class="row wrapper border-bottom white-bg page-heading">
    ...
    <div class="col-lg-2">
    	<a style="margin-top:20px;" href="<?php echo $this->Html->url(array('controller' => 'category', 'action' => 'admin_addsub')); ?>" class="btn btn-primary control-label pull-right">
    		Add
    	</a>
    </div>
</div>
...
<div id="dialog-confirm" title="Delete Category?" style="display:none;">
  <p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Are you sure You want to Delete this Sub-Category?</p>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#list").jqGrid({
			url: '<?php echo $this->Html->url(array("controller" => "category", "action" => "admin_subcategorylist", $maincatid)); ?>',
			datatype: "json",
			mtype: 'GET',
			colNames: ['S No','Sub-Category', 'Status', 'Edit', 'Delete'],
			colModel: [
	...
	function changeCategoryStatus(id,status){
		URL = '<?php echo $this->Html->url(array("controller" => "category","action" => "admin_changestatussub"));?>';
		$.ajax({
			url : URL,
			type: "POST",
		...
		$("#dialog-confirm").dialog({
			...
			buttons: {
		        "Delete Sub-Category": function() {
		        	$( this ).dialog( "close" );
					URL = '<?php echo $this->Html->url(array("controller" => "category","action" => "admin_deletesub"));?>';
					$.ajax({
						url : URL,
						type: "POST",
						data : ({id:id,status:status}),
